...
describe("formatModelName", () => {
	...
	it('shortens "claude-sonnet-4-5-*" to "sonnet-4.5"', () => {
		const result = formatModelName("anthropic/claude-sonnet-4-5-20250514", noAliases);
		expect(result).toBe("sonnet-4.5");
	});

	it("shortens sonnet-4.5 variant (dot separator)", () => {
		const result = formatModelName("anthropic/claude-4.5-sonnet-20250514", noAliases);
		expect(result).toBe("sonnet-4.5");
	});

	it('shortens "claude-sonnet-4-*" to "sonnet-4"', () => {
		const result = formatModelName("anthropic/claude-sonnet-4-20250514", noAliases);
		expect(result).toBe("sonnet-4");
	});

	it("returns 'sonnet' for generic claude sonnet without version", () => {
		const result = formatModelName("anthropic/claude-sonnet-latest", noAliases);
	...
	it('shortens claude opus to "opus"', () => {
		const result = formatModelName("anthropic/claude-opus-4-20250514", noAliases);
	...
	it('shortens claude haiku to "haiku"', () => {
		const result = formatModelName("anthropic/claude-3-haiku-20240307", noAliases);
	...
	it("preserves GPT-5 model string", () => {
		const result = formatModelName("openai/gpt-5-20250501", noAliases);
		expect(result).toBe("gpt-5-20250501");
	});

	it("preserves GPT-4 model string", () => {
		const result = formatModelName("openai/gpt-4-turbo-20241201", noAliases);
		expect(result).toBe("gpt-4-turbo-20241201");
	});

	it("preserves plain GPT-4 (no suffix)", () => {
		const result = formatModelName("openai/gpt-4", noAliases);
	...
	it('strips "-preview" suffix from gemini names', () => {
		const result = formatModelName("google/gemini-2.0-flash-exp-preview", noAliases);
		expect(result).toBe("gemini-2.0-flash-exp");
	});

	it("preserves gemini name without preview suffix", () => {
		const result = formatModelName("google/gemini-2.0-flash-001", noAliases);
	...
	it("truncates model IDs longer than 24 chars", () => {
		const result = formatModelName("some-very-long-model-name-that-exceeds-twenty-four", noAliases);
	...
	it("returns unknown model IDs with provider prefix without truncating", () => {
		const result = formatModelName("huggingface/my-awesome-model", noAliases);
